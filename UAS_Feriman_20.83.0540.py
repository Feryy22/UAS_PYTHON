...
def fetchnstore():
	...
	hasil = parsing['data']
	cursor = db.cursor()
	sql = "INSERT INTO tbl_students_0540 (id, nim, nama, jk, jurusan, alamat) VALUES (%s, %s, %s, %s, %s, %s)" 
	for row in hasil:
...
def show_all():
	cursor = db.cursor()
	sql = "SELECT * FROM tbl_students_0540"
	cursor.execute(sql)
	res = cursor.fetchall()
...
def with_limit():
	try:
		...
		sql = "SELECT * FROM tbl_students_0540 LIMIT %s" % (number)
		cursor.execute(sql)
		res = cursor.fetchall()
		print(tabulate(res,headers=["NO","NIM","Nama","JK","Jurusan","Alamat"],tablefmt='pretty'))
	except:
